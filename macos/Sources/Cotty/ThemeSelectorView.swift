...
/// Overlay theme selector view — positioned at top-center of window.
/// Dark semi-transparent background, search field, scrolling results list.
/// All filtering and theme data lives in Cot; Swift just renders.
class ThemeSelectorView: NSView {
    ...
    private func setupSearchField() {
        searchField.placeholderString = "Select a theme..."
    ...
    func dismiss() {
        cotty_theme_dismiss()
        isHidden = true
        workspaceController?.themeSelectorDidDismiss()
    }
    ...
    func reloadResults() {
        ...
        let count = Int(cotty_theme_result_count())
        let selected = Int(cotty_theme_selected())
        let maxShow = min(count, Self.maxResults)

        for i in 0..<maxShow {
            let titlePtr = cotty_theme_result_title(Int64(i))
            let titleLen = cotty_theme_result_title_len(Int64(i))
            var title = ""
            ...
            let row = ThemeRowView(title: title, isSelected: i == selected, index: i)
            row.target = self
    ...
    @objc private func searchFieldChanged(_ sender: Any?) {
        ...
        query.utf8.withContiguousStorageIfAvailable { buf in
            cotty_theme_set_query(buf.baseAddress, Int64(buf.count))
        }
        reloadResults()
    }

    @objc private func rowClicked(_ sender: Any?) {
        guard let row = sender as? ThemeRowView else { return }
        executeSelected(at: row.index)
    }

    func executeSelected(at index: Int? = nil) {
        let idx = index ?? Int(cotty_theme_selected())
        cotty_theme_apply(Int64(idx))
        dismiss()
        workspaceController?.applyThemeChange()
    }
...
extension ThemeSelectorView: NSTextFieldDelegate {
...
private class ThemeRowView: NSView {
